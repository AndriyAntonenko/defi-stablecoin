...
contract Collatarable is CommonModifiers {
  /*//////////////////////////////////////////////////////////////
                                 ERROR
  //////////////////////////////////////////////////////////////*/
  error Collatarable__WrongCollateral();
  error Collatarable__CollateralAndOraclesAddressesMustBeEqualLength();
  error Collatarable__InvalidOracle();
  error Collatarable__ZeroAddress();
  error Collatarable__TransferFailed();
  ...
  modifier allowedCollateral(address _collateral) {
    if (s_collateralOracles[_collateral] == address(0)) {
      revert Collatarable__WrongCollateral();
    }
    _;
  }
  ...
  constructor(
    ...
    if (_collaterals.length != _oracles.length) {
      revert Collatarable__CollateralAndOraclesAddressesMustBeEqualLength();
    }

    for (uint256 i = 0; i < _collaterals.length; i++) {
      if (_collaterals[i] == address(0) || _oracles[i] == address(0)) {
        revert Collatarable__ZeroAddress();
      }

      bool isOracleValid = AggregatorV3Interface(_oracles[i]).validateOracle();
      if (!isOracleValid) {
        revert Collatarable__InvalidOracle();
      }
    ...
    allowedCollateral(_collateral)
  {
    ...
    emit CallateralDeposited(msg.sender, _collateral, _amount);
    bool success = IERC20(_collateral).transferFrom(msg.sender, address(this), _amount);
    if (!success) {
      revert Collatarable__TransferFailed();
    }
  }
    ...
    moreThanZero(_amount)
  {
    ...
    emit CollateralRedeemed(_from, _to, _collateral, _amount);
